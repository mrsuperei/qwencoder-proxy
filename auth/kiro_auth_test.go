...
// kiroMockProxyClientFactory is a mock implementation of ProxyClientFactory for testing
type kiroMockProxyClientFactory struct {
	mockClient *http.Client
	calls      []*ProxyConfig
}

func (m *kiroMockProxyClientFactory) GetClient(proxyConfig *ProxyConfig) *http.Client {
...
func createKiroTestToken(id string) *ProviderToken {
	now := time.Now()
...
func createKiroTestTokenWithProxy(id string, proxyConfig *ProxyConfig) *ProviderToken {
	token := createKiroTestToken(id)
	token.Proxy = proxyConfig
	return token
}

func TestKiroAuthenticator_GetTokenWithClient(t *testing.T) {
	t.Run("returns token and proxy-aware client when token manager is set", func(t *testing.T) {
		...
		store := NewMultiTokenStore("kiro", filePath, logging.NewLogger())
		...
		token := createKiroTestTokenWithProxy("test-token-1", proxyConfig)
		...
		mockFactory := &kiroMockProxyClientFactory{mockClient: mockClient}
		...
		authenticator := NewKiroAuthenticator(nil)
	...
	t.Run("returns token with direct connection when token has no proxy", func(t *testing.T) {
		...
		store := NewMultiTokenStore("kiro", filePath, logging.NewLogger())
		token := createKiroTestToken("test-token-2")
		require.NoError(t, store.AddToken(*token))

		// Create token manager with mock proxy client factory
		mockFactory := &kiroMockProxyClientFactory{}
		proxyTracker := NewProxyHealthTracker(logging.NewLogger(), 5, 5*time.Minute)
		tokenManager := NewTokenManager(store, NewRandomSelectionStrategy(), logging.NewLogger(), mockFactory, proxyTracker)
...
func TestKiroAuthenticator_GetHTTPClient(t *testing.T) {
	t.Run("returns proxy-aware client when token manager is set", func(t *testing.T) {
		...
		token := createKiroTestTokenWithProxy("test-token-3", proxyConfig)
		require.NoError(t, store.AddToken(*token))
...
func TestKiroAuthenticator_BackwardCompatibility(t *testing.T) {
	t.Run("existing GetToken method still works", func(t *testing.T) {
		...
		store := NewMultiTokenStore("kiro", filePath, logging.NewLogger())
		token := createKiroTestToken("test-token-4")
		require.NoError(t, store.AddToken(*token))
	...
	t.Run("existing IsAuthenticated method still works", func(t *testing.T) {
		...
		// Create token store with a token
		store := NewMultiTokenStore("kiro", filePath, logging.NewLogger())
		token := createKiroTestToken("test-token-5")
		require.NoError(t, store.AddToken(*token))
