...
// Helper functions for proxy-aware tests
type iflowMockProxyClientFactory struct {
	mockClient *http.Client
	calls      []*ProxyConfig
}

func (m *iflowMockProxyClientFactory) GetClient(proxyConfig *ProxyConfig) *http.Client {
...
func createIFlowTestToken(id string) *ProviderToken {
	now := time.Now()
...
func createIFlowTestTokenWithProxy(id string, proxyConfig *ProxyConfig) *ProviderToken {
	token := createIFlowTestToken(id)
...
// Tests for proxy-aware functionality
func TestIFlowAuthenticator_GetTokenWithClient(t *testing.T) {
	t.Run("returns token and proxy-aware client when token manager is set", func(t *testing.T) {
		...
		store := NewMultiTokenStore("iflow", filePath, logging.NewLogger())
		...
		token := createIFlowTestTokenWithProxy("test-token-1", proxyConfig)
		...
		mockFactory := &iflowMockProxyClientFactory{mockClient: mockClient}
		...
		authenticator := NewIFlowAuthenticator(nil)
		...
		assert.Equal(t, "access-test-token-1", accessToken)
	...
	t.Run("returns token with direct connection when token has no proxy", func(t *testing.T) {
		...
		store := NewMultiTokenStore("iflow", filePath, logging.NewLogger())
		token := createIFlowTestToken("test-token-2")
		require.NoError(t, store.AddToken(*token))

		// Create token manager with mock proxy client factory
		mockFactory := &iflowMockProxyClientFactory{}
		proxyTracker := NewProxyHealthTracker(logging.NewLogger(), 5, 5*time.Minute)
		tokenManager := NewTokenManager(store, NewRandomSelectionStrategy(), logging.NewLogger(), mockFactory, proxyTracker)
		...
		require.NoError(t, err)
		assert.Equal(t, "access-test-token-2", accessToken)
		assert.NotNil(t, client)
		assert.Len(t, mockFactory.calls, 1)
...
func TestIFlowAuthenticator_GetHTTPClient(t *testing.T) {
	t.Run("returns proxy-aware client when token manager is set", func(t *testing.T) {
		...
		token := createIFlowTestTokenWithProxy("test-token-3", proxyConfig)
		require.NoError(t, store.AddToken(*token))
...
func TestIFlowAuthenticator_BackwardCompatibility(t *testing.T) {
	t.Run("existing GetToken method still works", func(t *testing.T) {
		...
		store := NewMultiTokenStore("iflow", filePath, logging.NewLogger())
		token := createIFlowTestToken("test-token-4")
		require.NoError(t, store.AddToken(*token))
		...
		require.NoError(t, err)
		assert.Equal(t, "access-test-token-4", accessToken)
	})
	...
	t.Run("existing IsAuthenticated method still works", func(t *testing.T) {
		...
		// Create token store with a token
		store := NewMultiTokenStore("iflow", filePath, logging.NewLogger())
		token := createIFlowTestToken("test-token-5")
		require.NoError(t, store.AddToken(*token))
