...
func TestGenerateContent_WithTokenManager(t *testing.T) {
	...
	resp, err := provider.GenerateContent(ctx, "glm-4.6", request)
...
func TestGenerateContentStream_WithTokenManager(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		w.Write([]byte("data: {\"id\":\"chatcmpl-123\",\"choices\":[{\"message\":{\"content\":\"Hello\"}}]}\n\n"))
	}))
	defer server.Close()
	...
	ctx := context.Background()
	stream, err := provider.GenerateContentStream(ctx, "glm-4.6", request)
...
func TestProxyHealthTracking(t *testing.T) {
	...
	base := provider.NewBaseProvider(logging.NewLogger(), 5*time.Minute)
	base.SetTokenManager(tokenManager)
	provider := &Provider{
		BaseProvider: base,
	}
...
func TestListModels(t *testing.T) {
	...
	modelsResp, ok := models.(*OpenAIModelsResponse)
	if !ok {
		t.Fatal("ListModels() did not return OpenAIModelsResponse")
	}
...
func TestNameAndProtocol(t *testing.T) {
	...
	if provider.Name() != "iflow" {
		t.Errorf("Expected name 'iflow', got '%s'", provider.Name())
	}

	if provider.Protocol() != "openai" {
		t.Errorf("Expected protocol 'openai', got '%s'", provider.Protocol())
	}
}
